...
現状、アプリケーションはReact + Vite構成で、全ての画面（ランディング、ログイン、サインアップ、ダッシュボード）を単一の状態管理で切り替えている。
 しかし、下記の理由により画面をURLに分離する必要がある。

#### 要件

- 各画面に意味のあるURLを割り当てること（例：`/login`, `/dashboard`）
- ブラウザの戻る・進むボタンで正しく画面が遷移すること
- ユーザーが画面ごとのURLをブックマーク・共有できるようにすること
- リロード時や直接アクセス時も正しく遷移先が表示されること
...
- `react-router-dom` latest (7.7.0) を使用

#### ルーティング構成（初期構成）

| パス         | コンポーネント  | 説明                             |
| ------------ | --------------- | -------------------------------- |
| `/`          | `<TopPage/>`    | 静的ランディングトップページ     |
| `/login`     | `<Login />`     | ログイン画面                     |
